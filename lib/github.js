...
const getOpenSourceRepos = async () => {
  ...
  let [, data] = await client.getAsync("/users/fu4303/repos", {
    per_page: 100,
    type: "public",
  });
  ...
  const commitPromises = data.map(async (repo) => {
    return await client.getAsync(`/repos/fu4303/${repo.name}/commits`, {
      per_page: 1,
    });
  });
  ...
  commitData = commitData
    ...
    .reduce((allCommitData, commit) => {
      const repoName = commit?.commit.url.match(/fu4303\/(.+)\/git/)[1];
  ...
  const tagPromises = data.map(async (repo) => {
    return await client.getAsync(`/repos/fu4303/${repo.name}/tags`, {
      per_page: 1,
    });
  });
  ...
  tagData = tagData
    ...
    .reduce((alltagData, tag) => {
      const repoName = tag.zipball_url.match(/fu4303\/(.+)\/zipball/)[1];
